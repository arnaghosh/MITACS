...
class motorLearningTask:
    ...
    def familiarization(self, score_thresh, trialTime, block, dThread):
        ...
        cv2.putText(task3, "Press Enter when ready",(int(self.width2/8),int(self.height2/3)),cv2.FONT_HERSHEY_PLAIN,4,(255,255,255),3);
        ...
        while( score< score_thresh):
            ...
            cv2.putText(task2, s,(int(self.width1/5),int(self.height1/3)),cv2.FONT_HERSHEY_PLAIN,4,(0,0,0),5);
            s = s+s1;
    ...
    def gripperTask(self, totalTrials, trialTime, block, dThread):         #trialTime in seconds
...
if __name__=='__main__':
    ...
    T_start = time.clock();
    if block==0:
        obj.familiarization(score_thresh, trialTime, block, dThread);
    else:
        obj.gripperTask(NumOfTrials,trialTime,block, dThread);
    T_end = time.clock();
    mutex.acquire();
