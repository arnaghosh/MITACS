...
# Each target is 0.08*width and error margin is +-2 %
import numpy as np
...
class Present_PERFORM_learn:
    ...
    def drawTarget(self,img1,img2):        
        ...
        cv2.rectangle(img1,(width1/8,height1/4),(3*width1/8,height1/2),(255,255,255),-1);
        cv2.rectangle(img2,(width2/8,height2/4),(3*width2/8,height2/2),(255,255,255),-1);        
        cv2.rectangle(img1,(5*width1/8,height1/8),(7*width1/8,3*height1/8),(255,255,255),-1)
        cv2.rectangle(img2,(5*width2/8,height2/8),(7*width2/8,3*height2/8),(255,255,255),-1)
        pts = np.array([[3*width1/8,height1/4],[3*width1/8,height1/2],[5*width1/8,3*height1/8],[5*width1/8,height1/8]], np.int32);
        pts = pts.reshape((-1,1,2));
        cv2.fillPoly(img1,[pts],(255,255,255));
        pts = np.array([[3*width2/8,height2/4],[3*width2/8,height1/2],[5*width2/8,3*height2/8],[5*width2/8,height2/8]], np.int32);
        pts = pts.reshape((-1,1,2));
    ...
    def gripperTask(self, totalTrials, trialTime, dThread):         #trialTime in seconds
        ...
        while( trialNo<= totalTrials):
            ...
            prev_x1 = 0.124*self.width1;
            prev_x2 = 0.124*self.width2;
            ...
            while(1):
                ...
                cv2.circle(taskImg1, (int(present_x1),int(present_y1)),30,(255,255,0),-1);
                cv2.circle(taskImg2, (int(present_x2),int(present_y2)),15,(255,255,0),-1);
                ...
                if (t11-t01)> trialTime:
                    break;
                inc_x1 = (0.75*self.width1-present_x1)*(t11-t_old)/(trialTime - t11+t01);
                inc_x2 = (0.75*self.width2-present_x2)*(t11-t_old)/(trialTime - t11+t01);
